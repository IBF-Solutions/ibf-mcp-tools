#!/usr/bin/env python3
"""Hetzner Cloud MCP Server -- standalone, integriert in IBF-Auth.

Adaptiert von dkruyt/mcp-hetzner (MIT License, https://github.com/dkruyt/mcp-hetzner).
Aenderungen gegenueber Original:
  - Auth-Gate via ibf_mcp_auth.Auth("hetzner")
  - Lazy-Token-Loading aus env / keyring / .env
  - Silent-unavailable wenn Token oder Auth fehlt
  - Eigenes --install/--uninstall/--set-token/--set-password CLI
  - Domain-Name 'hetzner' (statt 'Hetzner Cloud')
"""
...
# IBF-Auth-Modul. Wir liegen in claude_base/projekte/hetzner/tools/
# parents[0]=tools  [1]=hetzner  [2]=projekte  [3]=claude_base
# -> claude_base/tools/ibf_mcp_auth.py
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "tools"))
from ibf_mcp_auth import Auth  # noqa: E402

_auth = Auth("hetzner")


def _load_hcloud_token() -> str:
    """env > keyring > .env > leer."""
    token = os.environ.get("HCLOUD_TOKEN", "").strip()
    if token:
        return token
    try:
        import keyring
        token = keyring.get_password("hetzner", "ibf")
        if token:
            return token.strip()
    except Exception:
        pass
...
HCLOUD_TOKEN = _load_hcloud_token()
...
def _get_client():
    ...
    if _client is None:
        if not HCLOUD_TOKEN:
            return None
        _client = Client(token=HCLOUD_TOKEN)
    return _client
...
class _LazyClient:
    def __getattr__(self, name):
        c = _get_client()
        if c is None:
            raise RuntimeError("HCLOUD_TOKEN not configured")
        return getattr(c, name)


client = _LazyClient()

mcp = FastMCP("hetzner", instructions=(
    "Hetzner Cloud Management. WICHTIG: Pruefe bei JEDER Anfrage zuerst ob die Domain "
    "authentifiziert ist und HCLOUD_TOKEN gesetzt. Erhaeltst du '__UNAVAILABLE__' "
    "verhalte dich als gaebe es diesen MCP nicht (kein Erwaehnen, kein Passwort fragen). "
    "Vor destruktiven Aktionen (delete_*, power_off, set_firewall_rules) IMMER explizit "
    "den Nutzer fragen 'Soll ich [Resource] wirklich loeschen/aendern?' und auf "
    "Bestaetigung warten."
))
...
def authenticate(password: str) -> str:
    """Authentifizierung fuer die Hetzner-Domain.

    Globales IBF-Passwort schaltet alle IBF-MCPs frei. Hetzner-spezifisches Passwort
    schaltet nur diesen MCP frei.
...
@mcp.tool()
def list_servers(label_selector: Optional[str] = None) -> Dict[str, Any]:
    """
    List servers in your Hetzner Cloud account, optional gefiltert nach Labels.

    Args:
        label_selector: Optionaler Hetzner Label-Selector (z.B. 'env=prod',
                        'env in (dev,test)', 'ce-wissen' oder mehrere getrennt
                        per Komma 'env=prod,role=web'). Leer = alle Server.

    Beispiele:
        list_servers()                              -> alle
        list_servers(label_selector="ce-wissen")    -> nur Server mit Key ce-wissen
        list_servers(label_selector="env=prod")     -> nur Server mit env=prod
    """
    guard = _require_auth()
    if guard:
        return guard
    try:
        ...
        return {
            "filter": {"label_selector": label_selector} if label_selector else None,
            "count": len(servers),
...
@mcp.tool()
def list_firewalls(label_selector: Optional[str] = None) -> Dict[str, Any]:
    """
    List firewalls, optional gefiltert nach Labels.

    Args:
        label_selector: Optionaler Hetzner Label-Selector
                        (z.B. 'env=prod', 'role in (web,db)').
                        Leer = alle.
    """
    guard = _require_auth()
    if guard:
    ...
    try:
        if label_selector:
            items = client.firewalls.get_all(label_selector=label_selector)
        else:
            items = client.firewalls.get_all()
        return {
...
@mcp.tool()
def list_volumes(label_selector: Optional[str] = None) -> Dict[str, Any]:
    """
    List volumes, optional gefiltert nach Labels.
    ...
    try:
        if label_selector:
            items = client.volumes.get_all(label_selector=label_selector)
        else:
            items = client.volumes.get_all()
        return {
...
@mcp.tool()
def list_ssh_keys(label_selector: Optional[str] = None) -> Dict[str, Any]:
    """
    List ssh keys, optional gefiltert nach Labels.
    ...
    try:
        if label_selector:
            items = client.ssh_keys.get_all(label_selector=label_selector)
        else:
            items = client.ssh_keys.get_all()
        return {
...
# ---------------------------------------------------------------------------
# Generic label management (cross-resource)
# ---------------------------------------------------------------------------

class UpdateLabelsParams(BaseModel):
    resource_type: str = Field(..., description="server | volume | firewall | ssh_key")
    resource_id: int = Field(..., description="ID der Resource")
    labels: Dict[str, str] = Field(..., description="Labels als key=value Dict. Bei mode=remove werden Werte ignoriert -- nur Keys zaehlen.")
    mode: str = Field("merge", description="merge (default, additiv) | replace (ueberschreibt alles) | remove (entfernt diese Keys)")


@mcp.tool()
def update_labels(params: UpdateLabelsParams) -> Dict[str, Any]:
    """
    Labels einer Hetzner-Resource setzen / mergen / entfernen.

    Funktioniert fuer Server, Volumes, Firewalls und SSH-Keys.

    Args:
        resource_type: 'server', 'volume', 'firewall', 'ssh_key'
        resource_id:   numerische ID
        labels:        Dict {key: value}
        mode:
          - merge   (Default) -- bestehende Labels behalten, neue dazu/aktualisieren
          - replace          -- komplett ersetzen, alle alten Labels weg
          - remove           -- die genannten Keys aus den bestehenden Labels entfernen

    Beispiele:
        update_labels({"resource_type":"server","resource_id":12738181,
                       "labels":{"env":"prod"},"mode":"merge"})
        update_labels({"resource_type":"server","resource_id":12738181,
                       "labels":{"old-tag":""},"mode":"remove"})

    Returns: id, labels_before, labels_after
    """
    guard = _require_auth()
    if guard:
        return guard
    try:
        type_map = {
            ...
            "ssh_key":  client.ssh_keys,
        }
        rt = params.resource_type.lower()
        if rt not in type_map:
            return {"error": f"Unbekannter resource_type '{params.resource_type}' -- erlaubt: {', '.join(type_map.keys())}"}

        repo = type_map[rt]
        resource = repo.get_by_id(params.resource_id)
        if not resource:
            return {"error": f"{rt} mit ID {params.resource_id} nicht gefunden"}

        current = dict(resource.labels or {})

        mode = params.mode.lower()
        if mode == "merge":
            new_labels = {**current, **params.labels}
        elif mode == "replace":
            new_labels = dict(params.labels)
        elif mode == "remove":
            new_labels = {k: v for k, v in current.items() if k not in params.labels}
        else:
            return {"error": f"Unbekannter mode '{params.mode}' -- erlaubt: merge, replace, remove"}

        updated = repo.update(resource, labels=new_labels)
        return {
            "resource_type":  rt,
            "id":             updated.id,
            "name":           getattr(updated, "name", None),
            "labels_before":  current,
            "labels_after":   dict(new_labels),
            "mode":           mode,
        }
    except Exception as e:
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "--install":
            ...
            for scope in ("local", "user"):
                subprocess.run(["claude", "mcp", "remove", "hetzner", "-s", scope],
                               capture_output=True, text=True)
            r = subprocess.run(
                ["claude", "mcp", "add", "--scope", "user", "hetzner", "--", "python", script],
                capture_output=True, text=True,
            )
            if r.returncode == 0:
                print(f"[OK] MCP 'hetzner' registriert (user scope):\n  {r.stdout.strip()}")
            else:
            ...
            for scope in ("local", "user"):
                subprocess.run(["claude", "mcp", "remove", "hetzner", "-s", scope],
                               capture_output=True, text=True)
            print("[OK] MCP 'hetzner' entfernt.")

        elif cmd == "--set-token":
            import getpass, keyring
            token = getpass.getpass("HCLOUD_TOKEN (Hetzner Cloud API Token): ").strip()
            if not token:
                print("[FEHLER] Kein Token eingegeben.")
                sys.exit(1)
            keyring.set_password("hetzner", "ibf", token)
            print("[OK] Token gespeichert: Windows Credential Manager [hetzner/ibf]")

        elif cmd == "--set-password":
            import getpass
            pw = getpass.getpass("MCP-Passwort fuer 'hetzner': ").strip()
            if not pw:
                print("[FEHLER] Kein Passwort eingegeben.")
                sys.exit(1)
            pw2 = getpass.getpass("Wiederholen: ").strip()
            if pw != pw2:
                print("[FEHLER] Passwoerter nicht identisch.")
                sys.exit(1)
            _auth.set_password(pw, scope="local")
            print("[OK] Passwort fuer MCP 'hetzner' gespeichert.")

        elif cmd == "--test":
            print(f"HCLOUD_TOKEN: {'gesetzt' if HCLOUD_TOKEN else '(leer)'}")
            if not HCLOUD_TOKEN:
                print("Token noch nicht gesetzt -- mit --set-token einrichten.")
                sys.exit(1)
            try:
                _auth._write_token(_auth._token_file)
                print("Teste Hetzner-API ...")
                servers = _get_client().servers.get_all()
                print(f"[OK] {len(servers)} Server in der Cloud sichtbar.")
            except Exception as e:
                print(f"[FEHLER] {e}")
                sys.exit(1)
            ...
            print(f"  python {Path(script).name} --set-token      # HCLOUD_TOKEN ins Keyring")
            print(f"  python {Path(script).name} --set-password   # MCP-Passwort fuer 'hetzner'")
            print(f"  python {Path(script).name} --test           # Verbindung testen")
            print(f"  python {Path(script).name}                  # MCP-Server starten (fuer Claude)")
            sys.exit(1)
    else:
        mcp.run()
